...
Most of the actual code are found in helper files:
...
* helpers_train_predict -- Implement function for forward propagation 
  (from helpers_forward) and backward propagation (from helpers_backward) in
  the wrapper functions nn_predict() and nn_train(). 
  
  nn_train() is meant to be easily used out of the box; parameters such as 
  layers/nodes, learning rate, and iterations can be adjusted. However, 
  learning rate is constant throughout.
  
  nn_predict() is a function that uses the parameters from a trained model
  to make predictions on the classification of an image that it has not 
  seen before.
"""
...
os.chdir('../../Datasets/DeepLearningCoursera')
